export async function createKimiProvider(config) {
    let kimiSdk;
    try {
        kimiSdk = await import("@moonshot-ai/kimi-agent-sdk");
    }
    catch {
        throw new Error('Kimi provider requires @moonshot-ai/kimi-agent-sdk. Install it with: bun add @moonshot-ai/kimi-agent-sdk');
    }
    ...
    const externalTools = agentTools.map((t) => createExternalTool({
        name: t.name,
    ...
    for (const targetName of config.agent.handoffs ?? []) {
        ...
        externalTools.push(createExternalTool({
        ...
        for await (const event of turn) {
            ...
            if (event.type === "ToolResult") {
                yield {
                    type: "tool_result",
                    toolCallId: event.payload.id ?? "",
                    result: typeof event.payload.output === "string"
