...
export async function createOpenAIProvider(
  ...
  function buildTools(agent: AgentDef) {
    const tools: any[] = [];
  ...
  function findTool(agent: AgentDef, name: string): ToolDef | undefined {
    return agent.tools?.find((t) => t.name === name);
  }

  const messages: any[] = [];

  async function* runLoop(prompt: string): AsyncGenerator<StreamChunk> {
